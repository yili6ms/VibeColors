...
interface PaletteParams {
    bgSaturation: [number, number];
    bgLightness: [number, number];
    fgSaturation: [number, number];
    fgLightness: [number, number];
    accentSaturation: [number, number];
    accentLightness: [number, number];
}
...
function shiftRange(range: [number, number], delta: number, min = 0, max = 100): [number, number] {
    return [
...
function generateHarmonizedPalette(rng: () => number, isDark: boolean, style: PaletteStyle): ColorPalette {
    ...
    const baseParams: PaletteParams = isDark ? {
        bgSaturation: [15, 35],
        bgLightness: [8, 18],
        fgSaturation: [10, 30],
        fgLightness: [80, 95],
